...
import { ArrowLeft, Phone, Video, Info, Plus, Send, Mic, MessageCircle, MoreVertical, Smile, Check, CheckCheck, Paperclip, Image, Camera, MapPin, User as UserIcon, FileText, Download, Loader2, ChevronDown, Trash2, Share2, Forward, Copy, CheckSquare, PhoneIncoming, PhoneOutgoing, PhoneMissed, Star, Pin, Clock, Pause, Play, X, ZoomIn, Maximize, CornerUpLeft, AlertTriangle } from 'lucide-react';
...
const ChatArea = ({ contact, onBack, onHeaderClick, onSelectContact }) => {
    ...
    useEffect(() => {
        ...
        socket?.on('receive_message', (message) => {
            const isMatch = message.groupId
                ? (contact.isGroup && (message.groupId._id || message.groupId) === contact._id)
                : (!contact.isGroup && (message.sender._id === contact._id || message.recipient === contact._id));

            if (isMatch) {
                setMessages(prev => [...prev, message]);

                if (message.sender._id === contact._id || (!contact.isGroup && message.sender._id === contact._id)) {
                    socket.emit('mark_read', { messageIds: [message._id], senderId: message.sender._id });
                }
            }
        });
        ...
        socket?.on('typing', ({ senderId }) => {
            if (senderId === contact._id) setContactTyping(true);
        });

        socket?.on('stop_typing', ({ senderId }) => {
            if (senderId === contact._id) setContactTyping(false);
        });
    ...
    const fetchMessages = async () => {
        try {
            const targetId = contact.isGroup ? contact._id : (contact.contact?._id || contact._id);
            ...
            if (resp.ok) {
                ...
                setMessages([]);
                console.error('Fetch messages returned error:', data);
            }
        } catch (err) {
    ...
    const handleMicClick = async () => {
        if (isRecording) {
            ...
            try {
                ...
                alert('Cannot access microphone. Please ensure granted permissions.');
            }
        }
    };
    ...
    const handleSend = () => {
        ...
        try {
            ...
            if (contact.isGroup) {
                ...
                const targetId = contact.contact?._id || contact._id;
    ...
    const handleAttachmentUpload = async (file, type) => {
        try {
            const formData = new FormData();
            ...
            if (uploadData.fileId) {
                const tempId = Date.now().toString();
                const msgData = {
                    content: type === 'file' ? file.name : (type === 'image' || type === 'video' ? 'Media' : 'Audio'),
                    type: type,
                    fileId: uploadData.fileId,
                    fileMetadata: {
                        contentType: uploadData.contentType,
                ...
                if (contact.isGroup) {
                    ...
                    msgData.recipientId = contact._id;
                    socket?.emit('send_message', msgData, (response) => {
                        if (response?.messageId) {
                            setMessages(prev => prev.map(m => m._id === tempId ? { ...m, _id: response.messageId, status: response.status } : m));
                        }
    ...
    const handleSendCurrentLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition((position) => {
            ...
            const msgData = { recipientId: contact._id, content: q, type: 'location', clientId: tempId };
    ...
    const handleSendLiveLocation = (durationMinutes) => {
        ...
        navigator.geolocation.getCurrentPosition((pos) => {
            ...
            const msgData = { recipientId: contact._id, content: q, type: 'live_location', expiresAt, clientId: tempId };
            ...
            socket?.emit('send_message', msgData, (r) => {
                if (r?.messageId) {
                    ...
                    liveWatchIdRef.current = navigator.geolocation.watchPosition((newPos) => {
                        ...
                        socket?.emit('update_live_location', { messageId: r.messageId, content: newQ, recipientId: contact._id });
    ...
    const handleStopLiveLocation = (msgId) => {
        if (liveWatchIdRef.current) navigator.geolocation.clearWatch(liveWatchIdRef.current);
        socket?.emit('stop_live_location', { messageId: msgId, recipientId: contact._id });
    ...
    const handleMapSelect = (selectedData) => {
        ...
        const tempId = Date.now().toString();
        const msgData = { recipientId: contact._id, content: coords, type: 'location', clientId: tempId, address: addressText };
    ...
    return (
        <div className="chat-area">
            ...
            {showFullImage && (
                ...
                >
                    <img 
                        src={contact.profilePicture ? (contact.profilePicture.startsWith('http') ? contact.profilePicture : (import.meta.env.VITE_BACKEND_URL || "http://192.168.1.22:5000") + "/api/files/" + contact.profilePicture + "?token=" + localStorage.getItem('token')) : `https://ui-avatars.com/api/?name=${contact.displayName}&background=007AFF&color=fff`} 
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: '13px', color: contactTyping ? '#34C759' : 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {contact.isGroup
                                    ...
                                    : (contactTyping ? 'typing...' : (isOnline ? 'online' : 'offline'))
                                }
                            </div>
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', color: 'var(--primary)' }}>
                    ...
                    {showChatOptions && (
                        <>
                            ...
                            <div className="animate-fade" style={{ position: 'absolute', top: '45px', right: '8px', background: '#fff', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', overflow: 'hidden', minWidth: '160px', zIndex: 100 }}>
                                <div onClick={handleClearChat} style={{ padding: '12px 16px', color: '#FF3B30', cursor: 'pointer', fontSize: '16px', fontWeight: '500' }}>
                                    Clear Chat
                                </div>
                            </div>
                        </>
            ...
            <div className="chat-area-content" style={{ padding: '12px 14px 100px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                ...
                                >
                                    <div className="pill-bubble" style={{
                                        ...
                                        {msg.type === 'image' || msg.type === 'video' ? (
                                            ...
                                            const tileZoom = 14;
                                            const tileX = Math.floor((lon + 180) / 360 * Math.pow(2, tileZoom));
                                                            ...
                                                            <a href={`https://www.google.com/maps?q=${lat},${lon}`} target="_blank" rel="noreferrer" style={{ display: 'block', width: '100%', height: '100%', textDecoration: 'none' }}>
                                                                ...
                                                                <div style={{ position: 'absolute', bottom: '18px', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                                                    <div style={{ width: '32px', height: '32px', borderRadius: '50%', border: '2px solid white', overflow: 'hidden', boxShadow: '0 2px 6px rgba(0,0,0,0.3)', background: '#007AFF' }}>
                                                                        <img src={isMe ? (user.profilePicture ? (import.meta.env.VITE_BACKEND_URL || "http://192.168.1.22:5000") + "/api/files/" + user.profilePicture + "?token=" + localStorage.getItem('token') : `https://ui-avatars.com/api/?name=${user.displayName}&background=007AFF&color=fff`) : (contact.profilePicture ? (import.meta.env.VITE_BACKEND_URL || "http://192.168.1.22:5000") + "/api/files/" + contact.profilePicture + "?token=" + localStorage.getItem('token') : `https://ui-avatars.com/api/?name=${contact.displayName}&background=007AFF&color=fff`)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
                                                                    </div>
                                            ...
                                            : msg.type === 'contact' ? (
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '270px', padding: '4px 0' }}>
                                                    ...
                                                    <div style={{ position: 'relative', flexShrink: 0, marginLeft: '4px' }}>
                                                        <img
                                                            src={isMe ? (user.profilePicture ? (import.meta.env.VITE_BACKEND_URL || "http://192.168.1.22:5000") + "/api/files/" + user.profilePicture + "?token=" + localStorage.getItem('token') : `https://ui-avatars.com/api/?name=${user.displayName}&background=007AFF&color=fff`) : (contact.profilePicture ? (import.meta.env.VITE_BACKEND_URL || "http://192.168.1.22:5000") + "/api/files/" + contact.profilePicture + "?token=" + localStorage.getItem('token') : `https://ui-avatars.com/api/?name=${contact.displayName}&background=007AFF&color=fff`)}
                                                ...
                                                <span style={{ display: 'block', fontSize: '16.5px', lineHeight: '1.4', letterSpacing: '-0.2px' }}>{msg.content}</span>
                                            )}
                                        <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
                                            {/* Admin Saved Indicator next to time */}
                                            {(user.role === 'admin' || isAdmin) && downloadedMediaIds.has(msg._id) && (
                                                <CheckCheck size={13} color="#34B7F1" style={{ marginBottom: '-1px' }} />
                                            )}
                                            ...
                                            {isMe && (
                                                <div style={{ display: 'flex' }}>
                                                    {msg.status === 'read' ? <CheckCheck size={16} color="#34B7F1" /> : (msg.status === 'delivered' ? <CheckCheck size={16} color="#8E8E93" /> : <Check size={16} color="#8E8E93" />)}
                                                </div>
                                            )}
                                        </div>
            ...
            <div style={{
                ...
                {isMessagingRestricted || isAnnouncementOnly ? (
                    ...
                    <div className="animate-fade" style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }}>
                        <div onClick={() => { isCancelledRef.current = true; if (mediaRecorderRef.current) mediaRecorderRef.current.stop(); setIsRecording(false); setIsPaused(false); }} style={{ width: '44px', height: '44px', background: '#FFE5E5', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
                            <Trash2 size={22} color="#FF3B30" />
                        </div>
                        <div onClick={handleTogglePause} style={{ width: '44px', height: '44px', background: isPaused ? '#E8F2FF' : '#F2F2F7', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
                            {isPaused ? <Play size={22} color="#007AFF" /> : <Pause size={22} color="#8E8E93" />}
                        </div>
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '10px', background: '#FFFFFF', borderRadius: '24px', padding: '8px 16px', border: '0.5px solid #C7C7CC', height: '44px' }}>
                            <div className={`online-pulse ${isPaused ? 'paused-animation' : ''}`} style={{ width: '8px', height: '8px', background: '#FF3B30', borderRadius: '50%', flexShrink: 0 }}></div>
                            <span style={{ fontSize: '15px', fontWeight: '700', color: '#FF3B30', minWidth: '46px' }}>{formatTime(recordingDuration)}</span>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '3px', flex: 1 }}>
                                {[6, 14, 10, 18, 8, 20, 14, 18, 10, 16, 8, 20, 14, 10, 18, 8, 14, 20, 10, 16].map((h, i) => (
                                    <div key={i} className={`voice-pulse ${isPaused ? 'paused-animation' : ''}`} style={{ flex: 1, height: `${h}px`, background: '#007AFF', borderRadius: '2px', opacity: 0.7, animationDelay: `${(i * 0.08).toFixed(2)}s` }} />
                                ))}
                            </div>
                        </div>
                        <div onClick={handleMicClick} style={{ width: '44px', height: '44px', background: '#007AFF', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, paddingLeft: '2px' }}>
                    ...
                    <>
                        ...
                        <div style={{ flex: 1, background: '#FFFFFF', borderRadius: '24px', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '10px', minHeight: '48px', border: '0.5px solid #C7C7CC' }}>
                        ...
                        {showEmojiPicker && (
                            <div className="animate-fade" style={{ position: 'absolute', bottom: '90px', left: '12px', right: '12px', zIndex: 120 }}>
                                <MediaPicker onEmojiClick={insertEmoji} onGifClick={(url) => handleSendMedia(url, 'gif')} onStickerClick={(url) => handleSendMedia(url, 'sticker')} />
                            </div>
                        )}
                    </>
                )}
            ...
            {showAttachMenu && (
                <div className="animate-fade" style={{ position: 'absolute', bottom: '100px', left: '14px', background: 'white', borderRadius: '16px', boxShadow: '0 4px 25px rgba(0,0,0,0.15)', padding: '16px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', zIndex: 100 }}>
                    {[
                        { icon: FileText, label: 'Document', color: '#7f66ff', onClick: () => docInputRef.current?.click(), show: allowFiles },
                        { icon: Camera, label: 'Camera', color: '#ff2e74', onClick: startCamera, show: allowMedia },
                        { icon: Image, label: 'Gallery', color: '#c33af3', onClick: () => galleryInputRef.current?.click(), show: allowMedia },
                        { icon: Mic, label: 'Audio', color: '#ff8a00', onClick: () => audioInputRef.current?.click(), show: allowVoice },
                        { icon: MapPin, label: 'Location', color: '#007AFF', onClick: handleLocationOptionClick, show: true },
                        { icon: UserIcon, label: 'Contact', color: '#009de2', onClick: handleContactClick, show: true },
                    ].filter(opt => opt.show !== false).map((opt, i) => (
                        <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', cursor: 'pointer' }} onClick={opt.onClick}>
                            <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: opt.color, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 6px rgba(0,0,0,0.2)' }}>
                                <opt.icon size={26} color="white" />
                            </div>
                            <span style={{ fontSize: '13px', color: '#54656f', fontWeight: '500' }}>{opt.label}</span>
                        </div>
                    ))}
                </div>
            )}
            ...
            <input type="file" ref={galleryInputRef} accept="image/*,video/*" style={{ display: 'none' }} onChange={(e) => { if (e.target.files[0]) handleAttachmentUpload(e.target.files[0], e.target.files[0].type.startsWith('video') ? 'video' : 'image'); e.target.value = ''; }} />
            <input type="file" ref={cameraInputRef} accept="image/*,video/*" capture="environment" style={{ display: 'none' }} onChange={(e) => { if (e.target.files[0]) handleAttachmentUpload(e.target.files[0], e.target.files[0].type.startsWith('video') ? 'video' : 'image'); e.target.value = ''; }} />
                    ...
                    <div style={{ background: '#fff', borderRadius: '16px', width: '90%', maxWidth: '300px', padding: '20px' }} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                            {availableContacts.map((u, idx) => (
                                ...
                                >
                                    <img src={u.profilePicture ? (u.profilePicture.startsWith('http') ? u.profilePicture : (import.meta.env.VITE_BACKEND_URL || "http://192.168.1.22:5000") + "/api/files/" + u.profilePicture + "?token=" + localStorage.getItem('token')) : `https://ui-avatars.com/api/?name=${u.displayName}&background=007AFF&color=fff`} style={{ width: '40px', height: '40px', borderRadius: '50%' }} alt="" />
